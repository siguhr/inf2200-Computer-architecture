...
cache_c *make_cache(int cache_size, int block_size, int assoc, char *name, int write_policy)
{
...
void memory_init(int cache_size1, int block_size1, int assoc1, int cache_size2, int block_size2, int assoc2, int cache_size3, int block_size3, int assoc3, int write_policy)
{
  /* Initialize memory subsystem here. */
  L1_inst_cache = make_cache(cache_size1 * 1024, block_size1, assoc1, "L1_instruction_cache", write_policy);
  L1_data_cache = make_cache(cache_size2 * 1024, block_size2, assoc2, "L1_data_cache", write_policy);
  L2_unif_cache = make_cache(cache_size3 * 1024, block_size3, assoc3, "L2_unified_cache", write_policy);
...
// write-through
void memory_write(unsigned int address, data_t *data, int write_policy)
{
  printf("memory: write 0x%08x\n", address);
  //printf("write_policy: %d\n", write_policy);

  if (write_policy == 1)                                // Write-through policy
  {
  ...
  if (write_policy == 2)
  {
    ...
    if (cache_write(address, L1_data_cache) == 2)       // Write-back policy
    
    { 
      //printf("_______________Write down to lower memory______________\n");
      cache_write(address, L2_unif_cache);
    }
    else
    {
...
int cache_write(unsigned int address, cache_c *cache)
{
  ...
  for (int i = 0; i < cache->assoc; i++) 
  {
    ...
    if (set->block[i]->tag == tag)                                // Tag hit
    { 
      ...
      {                                                             //new data is written to cache block 
        //printf("______Wrote new data to empty block in same set(%s)_______\n", cache->name);
        
        set->block[i]->tag = tag;
        set->block[i]->valid = 1;
        cache->hit++;                                               // tag hit, valid miss
        cache->write_miss++;
        return 0;
      }
    ...
    else
    {
      ...
      { 
        cache->miss++;
        cache->write_miss++;                                        // not found in set
        cache->tot_set_miss++;
        
        //printf("Not found in set\n");
        //printf("______Replace data/empty block(%s)_______\n", cache->name);
        cache->replaced++;
...
void memory_finish(void)
{
  ...
  int L1_inst_total = (L1_inst_cache->miss + L1_inst_cache->hit);
  int L1_data_total = (L1_data_cache->miss + L1_data_cache->hit);
  int L2_unif_total = (L2_unif_cache->miss + L2_unif_cache->hit);
  printf("L1 inst total hit/miss: %d\n", L1_inst_total);
  printf("L1 data total hit/miss: %d\n", L1_data_total);
  printf("L2 unif total hit/miss: %d\n", L2_unif_total);
  float total_hit_miss = (L1_inst_total + L1_data_total + L2_unif_total);
  printf("Total hit/miss: %d\n\n", (int)total_hit_miss);
  
  printf("Hit and miss each cache\n");
  ...
  // hit ratio
  printf("Hit and miss each cache ratio\n");
  ...
  printf("######### Essential data #########  \n\n");
  fprintf(stdout, "Executed %lu instructions.\n\n", instr_count);

  printf("Relative hit ratio caches \n");
  float relative_hit_L1_I = L1_inst_cache->hit * 100 / total_hit_miss;
  printf("Relative hit L1 isnt: %f\n", (float)relative_hit_L1_I);
  float relative_hit_L1_D = L1_data_cache->hit * 100 / total_hit_miss;
  printf("Relative hit L1 data: %f\n", (float)relative_hit_L1_D);
  float relative_hit_L2 = L2_unif_cache->hit * 100 / total_hit_miss;
  printf("Relative hit L2 unif: %f\n", (float)relative_hit_L2);
  float relative_hit_tot = (float)relative_hit_L1_I + relative_hit_L1_D + relative_hit_L2;
  printf("Relative hit tot: %f\n\n", (float)relative_hit_tot);

  printf("Total cost-score: %f\n", total_amat);
  //printf("Hit total ratio: %f\n", (float)hit_ratio_total);
  printf("Relative hit tot: %f\n", (float)relative_hit_tot);
  printf("Relative miss tot: %f\n", (float) 100 - relative_hit_tot);
  printf("Policy: %d\n\n", L2_unif_cache->write_policy);
  printf("############## End ##############  \n\n");
